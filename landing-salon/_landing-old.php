...
$name = htmlspecialchars($brand['name'] ?? 'TeToca');
...
$ch = curl_init((schedulerApiUrl()) . '/services');
...
<footer>
    ...
    <p><?=$address?></p>
    <p style="margin-top:8px;">© <?=date('Y')?> <?=$name?> · Turnos online con TeToca</p>
</footer>
...
(function() {
    ...
    window.confirmarTurno = function() {
        ...
        phone = phone.replace(/\D/g, '').replace(/^54/, '').replace(/^0/, '').replace(/^(\d{2,4})15/, '$1');
        ...
        .then(function(r) {
            if (!r.ok) return r.json().then(function(err) { var msg = err.error || 'Error al crear el turno'; if (err.detail) msg += ' (' + err.detail + ')'; throw new Error(msg); });
            return r.json();
        })
        .then(function(data) {
